...
const { estDirecteur, peutVoirJournalPropre } = require('../middleware/permissions');
...
function parseDate(value, finDeJournee = false) {
  ...
  const date = new Date(`${value}T${finDeJournee ? '23:59:59.999' : '00:00:00.000'}`);
...
function dateEtHeure(date) {
  ...
  return {
    ...
    heure: valeur.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })
  };
}
...
async function exporterExcel(res, operations, filtres) {
  ...
  feuille.getCell('A1').fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF1F2937' } };
  feuille.getCell('A1').alignment = { horizontal: 'center' };
  feuille.mergeCells('A2:K2');
  feuille.getCell('A2').value = `Période : ${filtres.date_debut || 'Début'} au ${filtres.date_fin || "Aujourd'hui"} | Type : ${libelleType(filtres.type)}`;
  ...
  res.setHeader('Content-Disposition', 'attachment; filename="Historique_operations.xlsx"');
...
async function exporterPdf(res, operations, filtres) {
  ...
  res.setHeader('Content-Disposition', 'attachment; filename="Historique_operations.pdf"');
  document.pipe(res);

  document.font('Helvetica-Bold').fontSize(16).text('HISTORIQUE DES OPÉRATIONS', { align: 'center' });
  document.moveDown(0.3);
  document.font('Helvetica').fontSize(8).text(
  ...
  await document.table({
    headers: ['Date', 'Heure', 'Type', 'Site', 'Désignation', 'Qté', 'Montant'],
    rows
  }, {
...
async function afficherHistorique(req, res) {
  try {
    ...
    const typeDemande = String(req.query.type || 'tous').trim().toLowerCase();
    ...
    if (estDirecteur(req)) {
      ...
      if (
        ...
        if (!mongoose.Types.ObjectId.isValid(siteDemande)) {
          return res.status(400).json({ message: 'Identifiant de site invalide.' });
        }
    ...
    if (formatExport && !['excel', 'pdf'].includes(formatExport)) {
      return res.status(400).json({ message: "Format d'export invalide. Utilisez excel ou pdf." });
    }

    const limiteParDefaut = formatExport ? 5000 : 1000;
    const limite = Math.min(Math.max(Number(req.query.limite) || limiteParDefaut, 1), 5000);
    const besoin = (type) => typeDemande === 'tous' || typeDemande === type;

    const requetes = [
      besoin('stock')
        ? StockMouvement.find(filtreCommun).populate('site_id', 'nom ville').populate('user_id', 'username nom').lean()
        : Promise.resolve([]),
      (besoin('vente') || besoin('service'))
        ? Activite.find({ ...filtreCommun, type: { $in: ['vente', 'impression'] } }).populate('site_id', 'nom ville').populate('user_id', 'username nom').lean()
        : Promise.resolve([]),
      besoin('depense')
        ? Depense.find(filtreCommun).populate('site_id', 'nom ville').populate('user_id', 'username nom').lean()
        : Promise.resolve([]),
      besoin('depot')
        ? DepotBanque.find(filtreCommun).populate('site_id', 'nom ville').populate('user_id', 'username nom').lean()
        : Promise.resolve([]),
      (besoin('credit') || besoin('paiement_credit'))
        ? Credit.find(filtreCommun).populate('site_id', 'nom ville').populate('user_id', 'username nom').populate('paiements.user_id', 'username nom').lean()
        : Promise.resolve([])
    ];

    const [mouvementsStock, activites, depenses, depots, credits] = await Promise.all(requetes);
    const operations = [];
    ...
    mouvementsStock.forEach((item) => {
      ...
      operations.push({
        ...
        quantite: item.quantite_entree ?? item.quantite_unites,
    ...
    credits.forEach((item) => {
      if (besoin('credit') && dansPeriode(item.date_achat || item.createdAt, debut, fin)) {
        operations.push({
          _id: `credit:${item._id}`,
          source: 'credit',
          type: 'credit',
      ...
      if (besoin('paiement_credit') || typeDemande === 'tous') {
        (item.paiements || []).forEach((paiement) => {
          ...
          if (
            ...
            String(paiement.user_id?._id || paiement.user_id) !== String(filtreCommun.user_id)
          ) {
            return;
          }
          operations.push({
